...
@Composable
fun DeliveryNavigation(isHeaderAvailable: Boolean, navController: NavController) {
    Column(
        modifier = Modifier
            .fillMaxSize(),
        ...
        if(isHeaderAvailable) {
            DeliveryNavigationHeader(navController)
        }
    }
}
...
@Composable
fun DeliveryNavigationHeader(navController: NavController) {
    CenterAlignedTopAppBar(
        title = { Text(text = "배송 네비게이션", color = textColor) },
...
fun DeliveryNavigationPreview() {
    TwizyDeliveryAppTheme {
        DeliveryNavigation(true, rememberNavController())
    }
}